    ...
    },
    {
      ...
      content: (
        <div style={pageStyle}>
          ...
          <div style={{ borderTop: '1px solid #1e2a38', marginBottom: 14 }} />
          {[
            { q: 'That is more than we usually pay.',       a: 'I understand — my rate reflects specialist experience. Happy to scope a smaller project so you can see the value first.' },
            { q: 'Can you do it for a fixed price?',        a: 'Absolutely. For this scope I would price it at ' + proj + ' — everything included, no surprises.' },
            { q: 'We are comparing a few freelancers.',     a: 'I am not the cheapest, but clients return because I deliver on time and communicate clearly throughout.' },
          ...
          {[
            {
              ...
              body: 'Hi [Name],\n\nThanks for reaching out. I work at ' + rate + '/day — for this scope I would estimate 5 days, so around ' + proj + ' all in.\n\nAvailable from [date]. Does [time] work for a call?\n\nBest, [Your name]',
            },
            {
            ...
            },
            {
              ...
              body: 'Hi [Name],\n\nJust checking in on the proposal from [date]. I have a window opening that would work well for this.\n\nAny questions on the scope?\n\nBest, [Your name]',
            },
          ].map((t, i) => (
  ...
  return (
    <div style={{ background: '#f5f0e8', borderRadius: 16, padding: '24px', marginBottom: 32, border: '1px solid #e8e0d0' }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: '#6b5c45', marginTop: 4 }}>Browse all 5 sections before you decide. Your report uses your actual rates.</div>
      </div>
        ...
        <div style={{ display: 'flex', background: '#ede8de', borderBottom: '1px solid #d4c9b5', overflowX: 'auto' }}>
          {pages.map((p, i) => (
            <button
              key={i}
              onClick={() => setPage(i)}
              style={{ flex: 1, minWidth: 64, padding: '10px 6px', fontSize: 11, fontWeight: 700, background: 'none', border: 'none', borderBottom: page === i ? '2px solid #00e87a' : '2px solid transparent', color: page === i ? '#1a1a1a' : '#8b7355', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}
            >
              {p.label}
            </button>
          ))}
      ...
      <p style={{ fontSize: 12, color: '#8b7355', textAlign: 'center', marginTop: 8 }}>
