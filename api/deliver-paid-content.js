...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch their rate card from Supabase if we have an ID
    let rates = null
    let answers = null
...
function buildPremiumEmail(email, rates, answers) {
  const dayRate = rates?.dayRate || 'Your day rate'
  const projectRate = rates?.projectRate || 'Your project rate'
  const retainerRate = rates?.retainerRate || 'Your retainer rate'
  ...
  const specialty = answers?.specialty || answers?.[0] || 'your specialty'
  const yearsExp = answers?.experience || answers?.[1] || ''
  ...
  <div style="background:#ffffff;padding:32px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">
    ...
    <div style="background:linear-gradient(135deg,#0f1e35,#1a3a5c);border-radius:16px;padding:24px;margin-bottom:24px;">
      ...
      <div style="margin-bottom:16px;">
        ...
        <p style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;margin:0;">Always lead with your day rate or project rate — not your retainer. The first number anchors expectations. If you say a lower number first, everything else gets compared to it.</p>
      </div>

      <div style="margin-bottom:16px;">
        <p style="font-size:14px;font-weight:700;color:#ffffff;margin:0 0 6px;">2. Stop justifying your rate unprompted</p>
        <p style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;margin:0;">When you explain why you charge what you charge before anyone asks, you signal uncertainty. State your rate clearly, then pause. Let them respond first. Silence after quoting a rate is not rejection — it's processing.</p>
      </div>

      <div style="margin-bottom:16px;">
        <p style="font-size:14px;font-weight:700;color:#ffffff;margin:0 0 6px;">3. Raise by 15–20% with your next new client</p>
        <p style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;margin:0;">Your existing clients don't need to know. Your next new client has no reference point. The time to raise rates is with a new enquiry, not by renegotiating with someone already paying you. Aim for ${dayRate ? 'a 15% increase on ' + dayRate : 'a 15% increase on your current day rate'} with your next proposal.</p>
      </div>

      <div style="margin-bottom:16px;">
        <p style="font-size:14px;font-weight:700;color:#ffffff;margin:0 0 6px;">4. Reframe cost as value delivered</p>
        <p style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;margin:0;">If a client pushes back on price, ask: "What does this project need to achieve for it to be worth the investment?" This shifts the conversation from your cost to their outcome. Most clients cannot answer it quickly — and that pause resets the dynamic.</p>
      </div>

      <div style="margin-bottom:0;">
        <p style="font-size:14px;font-weight:700;color:#ffffff;margin:0 0 6px;">5. Review and raise every 6 months</p>
        <p style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;margin:0;">Set a calendar reminder for 6 months from today. At that point: if you haven't been turned down on price at least twice, your rates are too low. Rejection is data. Two rejections per month means your rates are right.</p>
      </div>
    </div>
    ...
    <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:12px;padding:16px;margin-bottom:0;">
      ...
      <p style="font-size:13px;color:#78350f;line-height:1.6;margin:0;">Set a reminder for 6 months from today to revisit your rates. Your rates should increase as your reputation grows. Two pricing rejections per quarter = healthy. Zero rejections = undercharging.</p>
    </div>
  </div>
