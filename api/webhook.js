...
function buildPDF(result) {
  return new Promise((resolve, reject) => {
    ...
    const positioning = (result && result.positioning) ? result.positioning : 'A trusted specialist who delivers clarity and results.'
    const script      = (result && result.script)      ? result.script      : 'My day rate is [rate]. That covers everything we have discussed. When would you like to get started?'
    ...
    const colW  = W / 3
    const grid  = [
      { l: 'Market Range', v: String(rangeLow) + ' – ' + String(rangeHigh) },
      { l: 'Project Rate', v: String(project) },
      { l: 'Retainer/mo',  v: String(retainer) },
      { l: 'Monthly',      v: String(monthly) },
      { l: 'Annual',       v: String(annual) },
      { l: 'Specialty',    v: String(specialty) },
    ]
    grid.forEach((c, i) => {
      const x = 56 + (i % 3) * colW
    ...
    const about = [
      ...
      ['Target income', String(income)],
      ['Client type',   String(clients)],
    ]
    ...
    doc.fontSize(13).fillColor(WHITE).font('Helvetica-Oblique')
       .text('"' + String(positioning) + '"', { lineGap: 5, width: W })
    ...
    doc.fontSize(12).fillColor(WHITE).font('Helvetica')
       .text('"' + String(script) + '"', 68, scriptY, { width: W - 24, lineGap: 5 })
    doc.moveDown(4)
    ...
    const objections = [
      {
        ...
        a: 'I understand — my rate reflects ' + String(experience) + ' of specialist experience in ' + String(specialty) + ' and the quality of output you will get. I am happy to scope a smaller initial project at ' + String(project) + ' so you can see the value first. Would that work?',
      },
      {
        q: 'Can you do it for a fixed price?',
        a: 'Absolutely. Based on the scope, I would price this at ' + String(project) + '. That covers everything we have discussed with no surprises — and you know the total cost upfront.',
      },
      {
    ...
    const templates = [
      {
        ...
        body: [
          ...
          'Hi [Name],',
          '',
          'Thanks for reaching out. Based on what you have described, this sounds like a great fit for my work in ' + String(specialty) + '.',
          '',
          'My day rate is ' + String(dayRate) + ', and for a project of this scope I would estimate 5 days, bringing the total to approximately ' + String(project) + '.',
          '',
      ...
      },
      {
        ...
        body: [
          'Subject: A note on my rates from [Month]',
          '',
          'Hi [Name],',
          '',
          'I wanted to give you advance notice that from [date] my day rate will be moving to ' + String(rangeHigh) + '.',
          '',
          'This reflects the increasing demand for my ' + String(specialty) + ' work and the rising cost of running my practice. Any projects agreed before [date] will remain at ' + String(dayRate) + '.',
          '',
      ...
      },
      {
        ...
        body: [
          ...
          'Hi [Name],',
          '',
          'Just checking in on the proposal I sent on [date] for ' + String(project) + '. I am keen to get started and have a window opening from [date] that would work well for this project.',
          '',
    ...
    const strategies = [
      'Raise for new clients first. Never touch existing client rates until you have tested ' + String(rangeHigh) + ' on 2 or 3 new projects successfully.',
      'Anchor high in discovery calls. Open with ' + String(rangeHigh) + ', then negotiate down if needed. Never open at ' + String(dayRate) + ' — that is your floor, not your ceiling.',
      'Stop itemising your time. Quote ' + String(project) + ' as a project price, not a day rate. This shifts the conversation from cost to value.',
      'Specialise visibly as a ' + String(specialty) + ' expert. The more specific your niche, the less you compete on price. Generalists get commoditised.',
      'Raise after every 3 projects. If you are winning ' + String(specialty) + ' work easily at ' + String(dayRate) + ', your rate is too low.',
      ...
      'Review retainers every 6 months. Your current retainer of ' + String(retainer) + '/month is a starting point, not a permanent rate.',
      'Document your outcomes. Rates go up when you can say "I delivered X result for a ' + String(clients) + ' client" not just "I did ' + String(specialty) + ' work".',
      'Let some clients go. When you raise to ' + String(rangeHigh) + ', lower-budget clients may fall away. That is positioning, not failure.',
      'Say your rate out loud daily. Before every call, say "My rate is ' + String(dayRate) + '/day." Confidence in quoting is a skill built through repetition.',
    ]
    ...
    const roadmap = [
      {
        month: 'Month 1–2',
        title: 'Consolidate at ' + String(dayRate),
        body: 'Charge ' + String(dayRate) + ' consistently across all new work. Do not discount. Track every project outcome — client name, result delivered, value created. This becomes your evidence base.',
      },
      {
        month: 'Month 3',
        title: 'Test the ceiling at ' + String(rangeHigh),
        body: 'Quote ' + String(rangeHigh) + ' to the next two new ' + String(clients) + ' clients. Do not explain or apologise. See what happens. Many freelancers are surprised to find it is accepted.',
      },
      {
        month: 'Month 4',
        title: 'Review your retainer clients',
        body: 'Write to any clients on a retainer. Your current benchmark is ' + String(retainer) + '/month. Give 60 days notice of a rate review. Frame it as a reflection of growing demand and improved service.',
      },
      {
        month: 'Month 5',
        title: 'Raise the project floor to ' + String(project),
        body: 'Stop accepting projects below ' + String(project) + '. Redirect smaller enquiries politely or decline. Use the freed capacity for higher-value work or business development.',
      },
      {
        month: 'Month 6',
        title: 'Full rate increase',
        body: 'Apply ' + String(rangeHigh) + ' as your new standard rate across all new work. Update your website, proposals, LinkedIn, and any platforms where your rate appears. You have earned it.',
      },
    ]
    ...
    doc.fontSize(40).fillColor(WHITE).font('Helvetica-Bold')
       .text('Your rate is ' + String(dayRate) + '.', 56, 200, { width: W })
    ...
    doc.moveDown(0.4)
    const summary = [
      ['Day rate',          String(dayRate)],
      ['Market range',      String(rangeLow) + ' – ' + String(rangeHigh)],
      ['Project rate',      String(project)],
      ['Monthly retainer',  String(retainer)],
      ['Specialty',         String(specialty)],
      ['Location',          String(location)],
    ]
...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    console.log('Webhook received — resultId:', resultId, 'email:', email)
    ...
    if (resultId) {
      ...
      if (updateError) {
        ...
        console.log('Supabase row marked as paid for resultId:', resultId)
      }
    }
    ...
    if (resultId) {
      ...
      if (fetchError) {
        ...
        result = data
        console.log('Fetched result from Supabase:', JSON.stringify(result, null, 2))
      }
    }

    if (email && result) {
      const appUrl  = process.env.VITE_APP_URL || 'https://claritycosts.co.uk'
      const dayRate = (result.rate && result.rate.dayRate) ? result.rate.dayRate : '—'
      ...
      try {
        await resend.emails.send({
          ...
          to: email,
          subject: 'Your Clarity Costs Full Toolkit — ' + String(dayRate) + '/day',
          attachments: pdfBuffer
            ? [{ filename: 'clarity-costs-full-toolkit.pdf', content: pdfBuffer.toString('base64') }]
            : [],
          html: `<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:48px 20px;background:#080b12;font-family:Inter,-apple-system,sans-serif;color:rgba(255,255,255,.88);">
  <div style="max-width:600px;margin:0 auto;">
    ...
    <p style="font-size:11px;color:rgba(255,255,255,.2);line-height:1.8;margin:0;">
      ...
      console.error('Missing email or result — email:', email, 'result:', result)
    }
  }
